...
\end{align*}
These seeds are the minimal set that simultaneously reproduces the classical circle and sphere while allowing a consistent backward extension. We retain the names ``surface'' and ``volume'' for \(\partial B_n\) and \(B_n\) by analogy with the seed cases, but stress that the multiplicative law~\eqref{eq:recurrence} is purely algebraic: it multiplies a ``surface'' by a ``volume'' to produce the next ``surface,'' an operation with no dimensional or geometric meaning beyond \(n=3\). The geometric content is confined to the seeds; everything above is algebra.
...
\item on the positive branch (\(n\ge 1\)) the integral relation \eqref{eq:integral} additionally holds, equivalently \(\partial B_n = \tfrac{d}{dr}B_n\).
\end{enumerate}
...
\begin{proof}
The first step was shown in the Introduction. Suppose the claimed forms hold up to index \(n\), with \(\deg_r \partial B_m = \deg_r B_m - 1\) for all \(m\le n\) (true for the seeds). Then
\[
\partial B_{n+1} = \partial B_n \cdot B_{n-1}
\]
is a monomial of radial degree \((e_n-1)+e_{n-1} = e_{n+1}-1 \ge 1\), using \(e_{n+1}=e_n+e_{n-1}\). In particular this exponent is never \(-1\), so the antiderivative is again a monomial and
\[
B_{n+1}(r)=\int_0^r \partial B_{n+1}(r')\,dr' = \frac{1}{e_{n+1}}\,(\text{coeff})\,r^{\,e_{n+1}}
\]
is well-defined, with \(\deg_r \partial B_{n+1} = e_{n+1}-1 = \deg_r B_{n+1}-1\), closing the induction. The displayed values for \(n=3,4,5\) are the first three instances.
\end{proof}

The radial degrees \(e_n\) (power of \(r\) in \(B_n\)) satisfy the \emph{pure Fibonacci} recurrence
\[
e_{n+1} = e_n + e_{n-1}, \qquad e_1 = 1,\ e_2 = 2,
\]
so that \(e_n = F_{n+1}\), the \((n{+}1)\)-th Fibonacci number. This is immediate from the multiplicative rule: the radial exponent of \(\partial B_{n+1}=\partial B_n\cdot B_{n-1}\) is the sum of the exponents of its two factors, and since on the positive branch \(\deg_r \partial B_m = \deg_r B_m - 1\) (the surface is the radial derivative of the volume), one has \(\deg_r \partial B_{n+1} = (e_n-1) + e_{n-1}\) and hence \(e_{n+1} = \deg_r \partial B_{n+1} + 1 = e_n + e_{n-1}\). The exponents therefore grow as \(\varphi^{\,n}\) (golden-ratio base), explaining the rapid increase visible in the table below; this is established in detail in Section~\ref{sec:stirling}.
...
\begin{proof}
Each step is an algebraic identity between rational functions (away from \(r=0\)), so it suffices to verify the finitely many cases in the tabulated range \(-10\le n\le 0\) by direct substitution; this is carried out in Section~5. No induction is required, since the range is finite.
\end{proof}

For the negative indices the integral relation \eqref{eq:integral} does \emph{not} reproduce the tabulated \(B_n\): at negative odd indices the integral diverges at the origin, while at negative even indices it converges but to a different monomial (e.g.\ \(\int_0^r \partial B_{-10}\,dr' = -r/(32\pi^4)\), whereas the recurrence-determined value is \(B_{-10}=-r^5/(32\pi^4)\)). The negative branch is therefore generated by the multiplicative recurrence and the chosen seeds \emph{alone}; the volume-from-surface integral (Convention~A) is a feature of the positive branch only. Equivalently, the identity \(\partial B_n = \tfrac{d}{dr}B_n\) holds for \(n\ge 1\) but fails for \(n\le -1\) (it holds incidentally only at \(n=-2\)). The listed \(B_n\) for \(n<0\) are the meromorphic functions \emph{determined by the chosen seeds} that keep the multiplicative recurrence satisfied throughout the whole integer line. (The recurrence alone does not pin them down; the seed choice does.) This is loosely analogous to the way analytic continuation via the Gamma function assigns finite values to expressions that diverge in the original integral representation.
...
\begin{proof}
The positive branch (\(n\geq 1\)) is generated precisely by the procedure of Proposition~\ref{prop:forward}; each multiplication step followed by integration preserves the relation by construction. For the negative branch it is sufficient to verify the algebraic identity
\[
...
V_n(r) = \frac{\pi^{n/2}}{\Gamma(n/2+1)}\, r^n .
\]
The two functions coincide for \(n=1,2,3\), where both reduce to \(2r\), \(\pi r^2\), and \(\tfrac43\pi r^3\) respectively. They differ for \emph{every} \(n\geq 4\). The discrepancy is structural rather than a matter of normalization: the radial degree of \(V_n\) is exactly \(n\), whereas the radial degree \(e_n\) of \(B_n\) obeys the pure Fibonacci recurrence \(e_{n+1}=e_n+e_{n-1}\) (Section~3) and equals the Fibonacci number \(F_{n+1}\),
\[
...
\label{eq:fibexp}
\end{equation}
where \(F_k\) is the \(k\)-th Fibonacci number (\(F_1=F_2=1\)). The radial-degree recursion is already the pure Fibonacci recursion \(e_{n+1}=e_n+e_{n-1}\) with \(e_1=1,\,e_2=2\), so \(e_n=F_{n+1}\) directly; the \(\pi\)-power obeys the same recursion shifted by two indices. Consequently the leading growth at unit radius is governed by the golden ratio:
\begin{equation}
\log B_n(1)\;\sim\;C\,\varphi^{\,n}, \qquad C = \lim_{n\to\infty}\frac{\log B_n(1)}{\varphi^{\,n}}\approx 0.23,
\label{eq:Bgrowth}
\end{equation}
with \(\varphi=(1+\sqrt5)/2\) the golden ratio. (The cruder estimate \(B_n(1)\approx\pi^{F_{n-1}}\) captures the dominant \(\pi\)-power but omits the rational prefactor, which decreases steadily and is \emph{not} negligible: at \(n=10\) it lowers \(\log_{10}B_n(1)\) from \(16.9\) to the true value \(12.5\). Equation~\eqref{eq:Bgrowth} uses the exact values and is the reliable asymptotic.) The empirical ratios \(e_n/e_{n-1}\) converge to \(\varphi\) (\(1.5,\,1.667,\,1.600,\,1.625,\dots\to 1.618\)).

\paragraph{The two laws side by side.} Equation \eqref{eq:stirling} decays like \(e^{-(n/2)\log n}\); equation \eqref{eq:Bgrowth} grows like \(e^{\,C\varphi^n}\) — a doubly-exponential increase driven by the golden ratio. The two families therefore separate not merely in magnitude but in the very nature of their growth, as Table~\ref{tab:asym} makes explicit. There is, in short, no asymptotic regime in which \(B_n\) approximates \(V_n\); they are driven by different special functions in opposite directions.
...
\midrule
10 & \(+0.4\)  & \(12.5\)      & \(\Gamma\) vs.\ golden-ratio growth \\
20 & \(-1.6\)  & \(\approx 1.5\times10^{3}\)  & \\
30 & \(-4.7\)  & \(\approx 1.9\times10^{5}\) & \\
\bottomrule
\end{tabular}
\end{table}
...
\bibitem{artin1964}
E.~Artin, \emph{The Gamma Function}, Holt, Rinehart and Winston, New York, 1964 (translated by M.~Butler); reprinted in \emph{Exposition by Emil Artin: A Selection}, Amer.\ Math.\ Soc., 2007. (Bohr--Mollerup theorem and the log-convex characterization of \(\Gamma\).)
